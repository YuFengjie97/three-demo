...
function Base() {
  const tex = useTexture(
    asset('/img/texture/particle/star_09.png'),
  ...
  useControls({
    ...
    velNoiseScale: {
      ...
      step: 0.01,
      onChange: (v) => {
        uniforms.uVelNoiseScale.value = v
      },
    },
    uHueOffsetScale: {
      value: 0.5,
      ...
      step: 0.01,
      onChange: (v) => {
        uniforms.uHueOffsetScale.value = v
      },
    },
  })
  ...
  const {positionAttr, getColor, computeInit, computeUpdate} = useMemo(() => {
    const positionBuffer = instancedArray(count, 'vec3')
    const velBuffer = instancedArray(count, 'vec3')
    const lifeBuffer = instancedArray(count, 'float')

    const positionAttr = positionBuffer.toAttribute()
    ...
    const getColor = Fn(() => {
      ...
      const hueOffset = dot(pos, vec3(0.1, 0.2, 0.3)).mul(
        uniforms.uHueOffsetScale,
      )
    ...
    return {positionAttr, getColor, computeInit, computeUpdate}
  }, [count])
